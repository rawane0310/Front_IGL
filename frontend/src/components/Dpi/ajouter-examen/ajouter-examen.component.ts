...
@Component({
  ...
  styleUrl: './ajouter-examen.component.css'
})
export class AjouterExamenComponent {
 formGroup = new FormGroup({
  ...
  constructor(public userIndicatorService: UserIndicatorsServiceServiceService ,  private dialogRef: MatDialogRef<AjouterExamenComponent>){
  ...
  async onSubmit(event: Event){
    ...
    if(this.formGroup.valid){
      try {
        ...
        const res = await axios.post('http://127.0.0.1:7000/examens/examens_radiologiques/',{
